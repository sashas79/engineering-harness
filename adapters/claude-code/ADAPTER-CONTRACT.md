...
- `.claude/settings.json` — a template. It carries the placeholders `{{harness_root}}` and `{{harness_core}}`, which `init` replaces with the values for the project being created: paths relative to the project directory when the Harness root and core sit beside it, absolute paths otherwise. The template also grants Claude Code permission to run the core command and denies the decision commands (`approve_pending`, `reject_pending`, `record_*_decision`), so the agent cannot record a human decision.
